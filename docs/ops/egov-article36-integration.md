...
> **法令注記**: e-Gov 電子申請の利用登録手順・認証方式・電子署名要否・申請様式の法的
> 解釈は、e-Gov 担当者および社労士・弁護士との一次法令確認が前提です。本ドキュメントは
> 法的助言ではありません。実運用前に必ず専門家の確認を取得してください。
...
- 電子署名 (電子証明書) の要否は申請種別によって異なる。
  証明書が必要な場合は `EGovRealConfig` に `CertPath` / `CertPassword` フィールドを追加し、
  Secret Manager で管理する。
